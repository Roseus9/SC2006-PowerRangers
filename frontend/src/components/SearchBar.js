...
function SearchBar() {
  ...
  const submitHandler = (e) => {
    const keyword_copy = keyword;
    const tags_copy = tags;
    setKeyword("");
    setTags("");
    e.preventDefault();
    if (keyword && tags) {
      navigate(`/?keyword=${keyword_copy}&tags=${tags_copy}`);
    } else if (keyword) {
      navigate(`/?keyword=${keyword_copy}`);
    } else if (tags) {
      navigate(`/?tags=${tags_copy}`);
    } else {
  ...
  return (
    <div>
      ...
      >
        ...
        <Button className="m-1" type="submit" variant="outline-success">
          Search
        </Button>
      </Form>
    </div>
